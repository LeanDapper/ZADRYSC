...
class PlayState extends MusicBeatState
{
	...
	public static var sickomode:Bool = false;
	...
	var scaredofdeathlol:Bool = false;
	var redtime:Bool = false;
	...
	var santa:FlxSprite;
	var zardybop:FlxSprite;
	var fence:FlxSprite;

	var insanetrail:FlxTrail;
	var redbg:FlxSprite;
	var stopit:Bool = false;
	var pumpkincrowd:FlxSprite;

	var jacknotzardy:FlxSprite;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'foolhardy':
				...
				{
					dialogue = CoolUtil.coolTextFile(Paths.txt('foolhardy/foolhardyDialogue2'));
				}
				else
				{
					dialogue = CoolUtil.coolTextFile(Paths.txt('foolhardy/foolhardyDialogue'));					
				}
			case 'robotic-link':
				dialogue = CoolUtil.coolTextFile(Paths.txt('robotic-link/thefunnydialogue'));
				
		}

		redtime = false;
		scaredofdeathlol = false;
		sickomode = false;
		done = false;
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'foolhardy' | 'zoned':
			{
				curStage = 'hell';
				defaultCamZoom = 0.55;
				var bg:FlxSprite = new FlxSprite(-1756.7, -824.6).loadGraphic(Paths.image('white_bg'));
				bg.antialiasing = true;
				bg.active = false;
				add(bg);

				jacknotzardy = new FlxSprite(291.25, -190.95);
				jacknotzardy.frames = Paths.getSparrowAtlas('jack_straw');
				jacknotzardy.animation.addByPrefix('hello', 'jack not zardy', 24);
				jacknotzardy.antialiasing = true;
				jacknotzardy.animation.play('hello');
				jacknotzardy.alpha = 0;
				if (SONG.song.toLowerCase() == 'foolhardy')
				{
					add(jacknotzardy);
				}				

				zardybop = new FlxSprite(-114.4, -210.65);
				zardybop.frames = Paths.getSparrowAtlas('zadry_crowd');
				zardybop.animation.addByPrefix('bop', 'zadry crowd', 24, false);
				zardybop.antialiasing = true;
				if (SONG.song.toLowerCase() == 'foolhardy')
				{
					add(zardybop);
				}

				fence = new FlxSprite(-2215.9, 247.2);
				fence.frames = Paths.getSparrowAtlas('funny_fence');
				fence.animation.addByPrefix('bop', 'fence', 24);
				fence.antialiasing = true;
				fence.animation.play('bop');
				add(fence);

				pumpkincrowd = new FlxSprite(-649.9, 590.7);
				pumpkincrowd.frames = Paths.getSparrowAtlas('dumb_pumpkins');
				pumpkincrowd.animation.addByPrefix('pumpkin', 'pumpkin foreground', 24);
				pumpkincrowd.antialiasing = true;
				pumpkincrowd.animation.play('pumpkin');

				redbg = new FlxSprite(-1756.7, -824.6).loadGraphic(Paths.image('red_bg'));
			...
			case 'robotic-link':
			{
				...
				var factorysetting:FlxSprite = new FlxSprite(-1623, -1268).loadGraphic(Paths.image('robotstage/olopep-stage'));
				factorysetting.scrollFactor.set(1, 1);
				factorysetting.active = false;
				add(factorysetting);
			}
			case 'senpai' | 'roses':
			{
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'hell':
				insanetrail = new FlxTrail(dad, null, 4, 10, 0.4, 0.065);
				gf.visible = false;
				boyfriend.setPosition(1553.4, 262.55);
				dad.setPosition(-900.2, 295.05);
			case 'factory':
				gf.visible = false;
				boyfriend.setPosition(1608.95, -33.55);
				dad.setPosition(-865.55, 420.2);			
		}
		...
		if (FlxG.save.data.beatedzadry == true && SONG.song.toLowerCase() == 'foolhardy')
		{
			doof.finishThing = ZadryAltIntro2;
		}
		else
		{
		...
		healthBar.createFilledBar(FlxColor.fromRGB(255, 153, 0), 0xFF66FF33);
		// healthBar
		add(healthBar);
	...
	function ZadryAltIntro(?dialogueBox:DialogueBox)
	{
		add(dialogueBox);
	}

	function FacilityIntro(?dialogueBox:DialogueBox)
		{
			FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));	
	...
	function ZadryAltIntro2()
	{
		...
		{
			dialogue = CoolUtil.coolTextFile(Paths.txt('foolhardy/foolhardyContinue'));
	...
	override public function update(elapsed:Float)
	{
		...
		{
			#if windows
			DiscordClient.changePresence("messing with the dang charts", null, null, true);
			#end
		...
		if (curSong == 'Foolhardy')
		{
			switch (curBeat)
			{
				...
				case 608:
				{
					camHUD.visible = true;
					sickomode = true;
					FlxTween.tween(dad, {alpha: 0.5}, 1.5);
					FlxTween.tween(boyfriend, {alpha: 0.5}, 1.5);
					FlxTween.tween(iconP2, {alpha: 0.5}, 1.5);
					FlxTween.tween(iconP1, {alpha: 0.5}, 1.5);
					FlxTween.tween(pumpkincrowd, {alpha: 0}, 1.5);
				...
				case 736:
					{
						...
						FlxTween.tween(redbg, {alpha: 0}, 1.5);
						FlxG.camera.shake(0.05, 0.7);
						FlxG.camera.flash(FlxColor.WHITE, 0.8);
					}
			}
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData))
						{
							case 2:
								...
								if (sickomode == true && FlxG.save.data.shakezadry == true)
								{
	...
	function endSong():Void
	{
		...
		else
		{
			...
			else
			{
				...
				if (curSong.toLowerCase() == 'foolhardy')
				{
					if (FlxG.save.data.beatedzadry == true)
					{
						if (FlxG.save.data.downloads == true)
						{
							FlxG.openURL("https://cdn.discordapp.com/attachments/559872850199183381/853495403345477642/creeper_ugh.mp3");
					...
					{
						FlxG.switchState(new BeatZadryScreen());
					}
					FlxG.save.data.beatedzadry = true;
				}
				else if (curSong.toLowerCase() == 'zoned')
				{
					FlxG.save.data.zonedcomplete = true;
					LoadingState.loadAndSwitchState(new VideoState('assets/videos/zadry ending.webm', new MainMenuState()));
				}
				else if (curSong.toLowerCase() == 'robotic-link')
				{
					LoadingState.loadAndSwitchState(new VideoState('assets/videos/RECIEVEDTRANSMISSION2030124.webm', new MainMenuState()));
				}
				else			
				{
					FlxG.switchState(new MainMenuState());
	...
	*/
	...
	function jackappear()
	{
		var jacksound:FlxSound = new FlxSound().loadEmbedded(Paths.sound('jacklaugh'));
		jacksound.volume = 2;
		FlxG.sound.list.add(jacksound);

		if (!jacksound.playing)
			jacksound.play(true);

		canjackappear = false;
		FlxTween.tween(jacknotzardy, {alpha: 1}, 0.2);
		new FlxTimer().start(2.5, function(tmr:FlxTimer)
			{
				FlxTween.tween(jacknotzardy, {alpha: 0}, 2);
	...
	override function beatHit()
	{
		...
		if (scaredofdeathlol == true && SONG.song == 'Foolhardy')
		{
			if (redbg.alpha == 0 && redtime == false)
			{
				FlxTween.tween(redbg, {alpha: 0.5}, 0.5);	
			}
			
		}
		else
		{
			if (redtime == false && SONG.song == 'Foolhardy')
				{
					FlxTween.tween(redbg, {alpha: 0}, 0.5);	
				}			
		}
		switch (curStage)
		{
			...
			case 'hell':
				zardybop.animation.play('bop', true);
				if (FlxG.random.bool(0.9) && canjackappear && redtime == false)
					jackappear();
				if (redtime == true && done == false)
				{
					if (stopit == false)
					{
						stopit = true;
